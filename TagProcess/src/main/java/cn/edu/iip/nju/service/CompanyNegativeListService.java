...
//    }
    ...
    /**
     * 分词器的误差导致没有分出市，需要手动修正
     */
    @Transactional
    public void addProvByCity() {
        ...
        map.put("襄阳","湖北省");
        map.put("汩罗","湖南省");
        List<CompanyNegativeList> list = companyNegativeListDao.findAllByProvinceAndAndCompanyNameLike("", "%市%");
        for (CompanyNegativeList c : list) {
            String companyName = c.getCompanyName();
            String prov = CityProvince.chooseProvinceOfCompany(companyName);
            if (!Strings.isNullOrEmpty(prov)) {
                c.setProvince(prov);
            }else{
                String city ;
                if(companyName.contains("省")){
                    city = companyName.substring(companyName.indexOf("省"), companyName.indexOf("市"));
                }else {
                    city = companyName.substring(0, companyName.indexOf("市"));
                }
                c.setProvince(map.get(city));
            }
            System.out.println(c.getProvince());
        }
        companyNegativeListDao.save(list);
    }
