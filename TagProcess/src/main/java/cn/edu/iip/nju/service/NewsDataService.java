...
//                System.out.println(injureCase);
    ...
    @Transactional
    public void removeAllRepeated() {
        List<NewsData> all = newsDataDao.findAll();
        Set<String> set = Sets.newHashSet();
        for (NewsData newsData : all) {
            if(set.contains(newsData.getUrl())){
                newsDataDao.delete(newsData.getId());
            }else{
                set.add(newsData.getUrl());
            }
        }
    }
