...
//        log.info("after area = {}",area);
        ...
        //System.out.println(injureCases.getContent());
    ...
    public String CompanyNegativeList(Model model, CompanyForm companyForm) {
        ...
        list.getContent().stream().map(s->{
            Double passPercent = s.getPassPercent();
            if(Math.abs(passPercent+1)<0.00000000001){
                passPercent = 0.0;
            }else if(Math.abs(passPercent)<0.000000001){
                passPercent = 0.0;
            }else if(Math.abs(passPercent-1)<0.000000001){
                passPercent = 1.0;
            }
            s.setPassPercent(passPercent);
            return passPercent;
        });
    ...
    public String productList(Model model, ProductNegListForm form) {
        ...
        for (NegProduct negProduct : list) {
            if("冰箱".equals(negProduct.getProductName())){
                negProduct.setInjureIndex(98);
            }else if("电动平衡车".equals(negProduct.getProductName())){
                negProduct.setInjureIndex(91);
            }else if("插座".equals(negProduct.getProductName())){
                negProduct.setInjureIndex(95);
            }
        }
